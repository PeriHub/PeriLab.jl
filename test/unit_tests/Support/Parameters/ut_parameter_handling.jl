...
@testset "ut_get_element_degree" begin
    ...
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_element_degree(Dict("Degree" => "ABC")))
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_element_degree(Dict("Degree" => "1")))
    @test PeriLab.Solver_control.Parameter_Handling.get_element_degree(Dict("Degree" => 1)) ==
          1
    @test PeriLab.Solver_control.Parameter_Handling.get_element_degree(Dict("Degree" => [
                                                                                1,
                                                                                2,
                                                                                3
                                                                            ])) == [1, 2, 3]
...
@testset "ut_get_element_type" begin
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_element_type(Dict()))
    @test PeriLab.Solver_control.Parameter_Handling.get_element_type(Dict("Element Type" => "ABC")) ==
          "ABC"
    @test PeriLab.Solver_control.Parameter_Handling.get_element_type(Dict("Element Type" => 12)) ==
          "12"
end

@testset "ut_get_output_type" begin
    @test PeriLab.Solver_control.Parameter_Handling.get_output_type(Dict("Output1" => Dict()),
                                                                    "Output1") == "Exodus"
    @test PeriLab.Solver_control.Parameter_Handling.get_output_type(Dict("Output1" => Dict("Output File Type" => "CSV")),
                                                                    "Output1") == "CSV"
    @test PeriLab.Solver_control.Parameter_Handling.get_output_type(Dict("Output1" => Dict("Output File Type" => "Exodus"),
...
@testset "ut_get_bond_filters" begin
    ...
    check, bfList = PeriLab.Solver_control.Parameter_Handling.get_bond_filters(params)
    @test !check
...
@testset "ut_validate_yaml" begin
    ...
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.validate_yaml(params))
    ...
    params = Dict{Any,Any}("PeriLab" => Dict{Any,Any}("Models" => Dict{Any,Any}("Material Models" => Dict{Any,
                                                      ...
                                                      "Solver" => Dict{Any,Any}("Final Time" => 1.0,
                                                                                "Initial Time" => 0.0)))
    @test PeriLab.Solver_control.Parameter_Handling.validate_yaml(params) ==
          params["PeriLab"]
end

@testset "ut_get_external_topology_name" begin
    params = Dict("Discretization" => Dict())
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_external_topology_name(params,
                                                                                         ""))
...
@testset "ut_get_mesh_name" begin
    params = Dict("Discretization" => Dict())
    @test PeriLab.Solver_control.Parameter_Handling.get_mesh_name(params) === nothing
    name = randstring(12)
    params = Dict("Discretization" => Dict("Input Mesh File" => name))
    @test PeriLab.Solver_control.Parameter_Handling.get_mesh_name(params) == name
end

@testset "ut_get_output_filenames" begin
    params = Dict()
    filenames = PeriLab.Solver_control.Parameter_Handling.get_output_filenames(params, "")
    @test filenames == []
...
@testset "ut_get_output_fieldnames" begin
    ...
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_output_fieldnames(outputs,
                                                                                    variables,
                                                                                    computes,
                                                                                    output_type))
...
@testset "get_output_frequency" begin
    ...
    params = Dict("Outputs" => Dict("Output1" => Dict("Output Frequency" => 2),
                                    "Output2" => Dict("Number of Output Steps" => 1,
                                                      "Output Frequency" => 1)))
    freq = PeriLab.Solver_control.Parameter_Handling.get_output_frequency(params, nsteps)
    @test freq[1] == 2
    @test freq[2] == 40
...
@testset "ut_get_computes" begin
    params = Dict()
    testfield_keys = test_data_manager.get_all_field_keys()
    @test PeriLab.Solver_control.Parameter_Handling.get_computes(params, testfield_keys) ==
          Dict()
    ...
    computes = PeriLab.Solver_control.Parameter_Handling.get_computes(params,
                                                                      testfield_keys)
...
@testset "ut_get_computes_names" begin
    ...
    computes_names = PeriLab.Solver_control.Parameter_Handling.get_computes_names(params)
...
@testset "ut_get_output_variables" begin
    @test PeriLab.Solver_control.Parameter_Handling.get_output_variables("Forces",
                                                                         [
                                                                             "Forces",
                                                                             "DisplacementsNP1"
                                                                         ]) == "Forces"
    @test PeriLab.Solver_control.Parameter_Handling.get_output_variables("Displacements",
                                                                         [
                                                                             "Forces",
                                                                             "DisplacementsNP1"
                                                                         ]) ==
...
@testset "ut_block_values" begin
    ...
    @test isnothing(PeriLab.Solver_control.Parameter_Handling._get_values(params, 1,
                                                                          "Density"))
    @test isnothing(PeriLab.Solver_control.Parameter_Handling._get_values(params, 1,
                                                                          "not there"))
    ...
    @test isnothing(PeriLab.Solver_control.Parameter_Handling._get_values(params, 2,
                                                                          "Density"))
    params = Dict("Blocks" => Dict("block_1" => Dict("Block ID" => 1,
                                                     "Density" => 1,
    ...
    @test PeriLab.Solver_control.Parameter_Handling._get_values(params, 2, "Density") ==
          12.3
    @test isnothing(PeriLab.Solver_control.Parameter_Handling._get_values(params, 3,
                                                                          "Density"))
    @test PeriLab.Solver_control.Parameter_Handling._get_values(params,
                                                                1,
                                                                "Specific Heat Capacity") ==
          3
    @test isnothing(PeriLab.Solver_control.Parameter_Handling._get_values(params,
                                                                          2,
                                                                          "Specific Heat Capacity"))
    @test isnothing(PeriLab.Solver_control.Parameter_Handling._get_values(params,
                                                                          3,
    ...
    @test PeriLab.Solver_control.Parameter_Handling._get_values(params,
                                                                3,
                                                                "Specific Heat Capacity") ==
          PeriLab.Solver_control.Parameter_Handling.get_heat_capacity(params, 3)
end
...
@testset "ut_solver" begin
    ...
    @test PeriLab.Solver_control.Parameter_Handling.get_numerical_damping(params["Solver"]) ==
    ...
    @test PeriLab.Solver_control.Parameter_Handling.get_nsteps(Dict("Verlet" => Dict("Safety Factor" => 0.95,
                                                                                     "Numerical Damping" => 5e-6),
                                                                    "Number of Steps" => 6)) ==
          6
    @test PeriLab.Solver_control.Parameter_Handling.get_numerical_damping(params["Solver"]) ==
          0.0
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_initial_time(Dict("Solver" => Dict()),
                                                                               test_data_manager))
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_final_time(Dict("Solver" => Dict()),
                                                                             test_data_manager))
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_final_time(Dict("Solver" => Dict()),
                                                                             test_data_manager))
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_solver_name(Dict("Solver" => Dict("Solvername" => Dict()))))
    params = Dict("Solver" => Dict("Initial Time" => 0.0,
                                   "Final Time" => 1.0,
...
@testset "ut_get_model_parameter" begin
    ...
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_model_parameter(params,
    ...
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.get_model_parameter(params,
                                                                                  "Material Model",
                                                                                  "s"))
    testData["Material Model"] = PeriLab.Solver_control.Parameter_Handling.get_model_parameter(params,
                                                                                               "Material Model",
                                                                                               "A")
    ...
    testData["Damage Model"] = PeriLab.Solver_control.Parameter_Handling.get_model_parameter(params,
                                                                                             "Damage Model",
                                                                                             "E")
...
@testset "ut_check_for_duplicates" begin
    @test !(PeriLab.Solver_control.Parameter_Handling.check_for_duplicates(["a", "b", "c"]))
    @test isnothing(PeriLab.Solver_control.Parameter_Handling.check_for_duplicates([
                                                                                       "a",
                                                                                       "b",
                                                                                       "c",
                                                                                       "a"
