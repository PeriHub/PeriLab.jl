...
@testset "ut_global_value_sum" begin
    ...
    (forcesN, forcesNP1) = test_data_manager.create_node_vector_field("Forces", Float64, 3)
    forcesNP1[1, 1:3] .= 1:3
    forcesNP1[3, 1:3] .= 5.2
    forcesNP1[4, 1] = -20
    ...
    disp = test_data_manager.create_constant_node_scalar_field("Disp", Float64)
    disp[1:4] = 1:4
    nodes = Vector{Int64}(1:4)
    ...
    matrix = test_data_manager.create_constant_node_tensor_field("Matrix", Float64, 3)
    matrix[:, 1, 2] .= 4
    nodes = Vector{Int64}(1:2)
