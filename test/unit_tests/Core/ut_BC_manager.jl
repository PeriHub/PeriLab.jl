...
@testset "ut_apply_bc" begin
    ...
    @test sum(disp) == 0
    bcs = PeriLab.Solver.Boundary_conditions.init_BCs(params, test_Data_manager)
    PeriLab.Solver.Boundary_conditions.apply_bc(bcs, test_Data_manager, 0.0)
    force = test_Data_manager.get_field("Forces")
    disp = test_Data_manager.get_field("Displacements", "NP1")
    @test sum(force) == 0
    @test sum(disp) == 20
    ...
    PeriLab.Solver.Boundary_conditions.apply_bc(bcs, test_Data_manager, 0.2)
    force = test_Data_manager.get_field("Forces")
    disp = test_Data_manager.get_field("Displacements", "NP1")
    @test sum(force) == 12
    ...
    bcs = PeriLab.Solver.Boundary_conditions.init_BCs(params, test_Data_manager)
    @test isnothing(PeriLab.Solver.Boundary_conditions.apply_bc(bcs, test_Data_manager, 0.2))
end