...
function compute_stretch!(stretch::BondScalarState{Float64},
                          deformed_bond_length::BondScalarState{Float64},
                          undeformed_bond_length::BondScalarState{Float64})
    sub_in_place!(stretch, deformed_bond_length, undeformed_bond_length)
    div_in_place!(stretch, stretch, undeformed_bond_length)
end
...
function compute_deformation_gradients!(deformation_gradient::NodeTensorField{Float64},
    ...
    for iID in nodes
        if deformed_bond[iID] == undeformed_bond[iID]
            @inbounds for i in 1:dof, j in 1:dof
                deformation_gradient[iID, i, j] = (i == j) ? 1.0 : 0.0
            end

            continue
        end
        @views compute_deformation_gradient!(temp,
                                             bond_damage[iID],
                                             deformed_bond[iID],
                                             undeformed_bond[iID],
                                             volume[nlist[iID]],
        ...
        @views mul!(dg, temp, inverse_shape_tensor[iID, :, :])
    end
end
        ...
        @inbounds @fastmath for m in axes(deformation_gradient, 2)
            @inbounds @fastmath for n in axes(deformation_gradient, 3)
                ...
                @views @inbounds @fastmath for (jID, nID) in enumerate(nlist[iID])
                    deformation_gradient[iID, n,
                                         m] += (displacement[nID, m] -
                                                displacement[iID, m]) *
...
function deformation_gradient_decomposition(nodes::Union{Base.OneTo{Int64},Vector{Int64}},
    ...
    for iID in nodes
        ...
        @views rot_tensor[iID, :,
                          :] = invert(rot_tensor[iID, :,
                                                 :],
...
function compute_linear_strain!(nodes::AbstractVector{Int64},
    ...
    for iID in nodes
        @inbounds for i in axes(strain, 2), j in axes(strain, 3)
            strain[iID, i,
                   j] = 0.5 * (deformation_gradient[iID, i, j] +
...
function compute_bond_level_rotation_tensor(nodes,
    ...
    for iID in nodes
        ba_rotation_tensor[iID][:, :,
                                :] = deformation_gradient_decomposition(eachindex(nlist[iID]),
