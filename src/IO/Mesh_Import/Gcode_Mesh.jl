...
This also removes whitespace as it is not needed for further parsing.
"""
...
function parseFile(path::String, callbacks::Dict{String,Function}, dataObject, silent)
    ...
    for i in iter
        ...
        elseif occursin(";", x)
            ...
            # commands themselves. A line consisting only of these (e.g. "B0 C0"
            # right after a G1 move, common in modal g-code that doesn't repeat
            # the G-word on every line) reuses the last motion mode.
...
function write_mesh(gcode_file, commands_dict,
    ...
    # here a simple dictionary is used to store information during the print
...
G02/G03 arc interpolation with extrusion.
"""
...
Variant of `write_pd_mesh` for arc segments.
"""
