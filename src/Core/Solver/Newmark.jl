...
mutable struct NewmarkCache
    ...
    a_n::Vector{Float64}   # acceleration at tₙ
    v_n::Vector{Float64}   # velocity at tₙ

    function NewmarkCache(n::Int)
        new(Float64[], BitVector(undef, n), Float64[], Float64[],
            nothing, Int[], zeros(n), zeros(n))
    end
end
...
Default: average acceleration (beta=0.25,gamma=0.5, unconditionally stable).
With numerical damping alpha: beta = 0.25(1+alpha)²,gamma = 0.5+alpha (HHT-alpha).
"""
function init_solver(solver_options::Dict{Any,Any},
    ...
    alpha = get_numerical_damping(params)
    solver_options["Newmark Beta"] = get(params, "Newmark Beta", 0.25 * (1 + alpha)^2)
    solver_options["Newmark Gamma"] = get(params, "Newmark Gamma", 0.5 + alpha)
...
function run_solver(solver_options::Dict{Any,Any},
    ...
    gamma::Float64 = solver_options["Newmark Gamma"]
    ...
    a0 = 1.0 / (beta * dt^2)        # M scaling in K_eff
    a1 = 1.0 / (beta * dt)          # v_n coefficient
    a2 = 1.0 / (2 * beta) - 1.0        # a_n coefficient
    a6 = dt * (1.0 - gamma)          # v update: a_n weight
    a7 = gamma * dt                   # v update: a_{n+1} weight

    @info "Newmark: beta=$beta,gamma=$gamma, dt=$dt"
    ...
    for idt in iter
        ...
        @timeit "Newmark step" begin
            ...
            vel = Data_Manager.get_field("Velocity", "NP1")
            ...
            @timeit "newmark_solve" newmark_step!(K, M, non_BCs,
                                                  ...
                                                  cache, a0, a1, a2, a6, a7, nnodes, dof)
...
#
#   K_eff = K + a0·diag(M)
#   F_eff = F_ext - F_int + M·(a0·uₙ + a1·vₙ + a2·aₙ)
#
# Then:
#   a_{n+1} = a0·(u_{n+1} - uₙ) - a1·vₙ - a2·aₙ
#   v_{n+1} = vₙ + a6·aₙ + a7·a_{n+1}
# ────────────────────────────────────────────────────────────
function newmark_step!(K::AbstractMatrix{Float64},
    ...
    @inbounds for (idx, i) in enumerate(non_BCs)
        m_rhs = M[i] * (a0 * u_n_vec[i] + a1 * cache.v_n[i] + a2 * cache.a_n[i])
    ...
    if has_BCs
        ...
        # Mass from prescribed DOFs (lumped → only diagonal, no coupling)
        @inbounds for (idx, i) in enumerate(non_BCs)
            # nothing extra needed for lumped mass with prescribed DOFs
            # since M is diagonal and the prescribed DOF contribution
            # only appears in the force for that DOF, not cross-coupled
        end
    end
    ...
    @inbounds for (idx, i) in enumerate(non_BCs)
        u_vec[i] = cache.u_free[idx]
    end

    # ── Update acceleration and velocity ──
    @inbounds for i in 1:n_total
        a_new = a0 * (u_vec[i] - u_n_vec[i]) - a1 * cache.v_n[i] - a2 * cache.a_n[i]
        cache.v_n[i] = cache.v_n[i] + a6 * cache.a_n[i] + a7 * a_new
        cache.a_n[i] = a_new
    end

    v_vec = vec(vel)
    copyto!(v_vec, cache.v_n)
